...
function updatePowerups(index, state) {
	powerUps.children[index].alive = state.alive
	powerUps.children[index].visible = state.visible
}
...
var eurecaClientSetup = function() {
	...
	eurecaClient.exports.setId = function(id) {
		...
		myId = id;
		create();	//runs game.create() for me, creates world and player
		eurecaServer.handshake(myId);
		ready = true;	//ready must be true for update() to run
	};
...
var game = new Phaser.Game(4000, 3000, Phaser.CANVAS, 'phaser-example', {
	preload: preload,
...
function preload() {
	...
	game.load.image('riflePowerup', 'assets/AK.png');
}
...
function update() {
	...
	//do something similar for powerup status
